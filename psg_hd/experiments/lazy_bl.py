"""
Run projected subgradient on Bernoulli-Laplace model with OPTIMIZED tensor product.

This version uses compute_tensor_product_vectorized for d=10 experiments.
Matches the structure and output format of projected_subgradient/experiments/run_bl_model.py
"""
import torch
import sys
...
def run_experiment(d=10, n=5, n_iter=None, s=1, epsilon=None, max_iter_cap=300, latex=False):
    """
    Run optimized projected subgradient on Bernoulli-Laplace level model.
    ...
    # Build family of matrices: P, P², P⁴, 0.25I + 0.75P, 0.5I + 0.5P, 0.75I + 0.25P
    ...
    # P
    P_list.append(P_base.clone())
    pi_list.append(pi_base)
    
    # P²
    P_list.append(P_base @ P_base)
    pi_list.append(pi_base)
    
    # P⁴
    P_list.append(P_base @ P_base @ P_base @ P_base)
    pi_list.append(pi_base)
    
    # 0.25I + 0.75P
    P_list.append(0.25 * I + 0.75 * P_base)
    pi_list.append(pi_base)
    
    # 0.5I + 0.5P
    P_list.append(0.5 * I + 0.5 * P_base)
    pi_list.append(pi_base)
    
    # 0.75I + 0.25P
    P_list.append(0.75 * I + 0.25 * P_base)
    pi_list.append(pi_base)
    ...
    logger = _get_logger()
    logger.save_config({
        'model': 'bernoulli_laplace_level_spectral',
        'd': d,
        'swap_size_s': s,
        'n': n,
        'n_iter': n_iter,
    ...
    axes[0].set_title(f"Bernoulli-Laplace Level Model (d={d}): Trajectory Plot of h(w)", pad=8)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run optimized PSG on Bernoulli-Laplace model')
    parser.add_argument("--d", type=int, default=5, help="Dimension (default: 5)")
    parser.add_argument("--n", type=int, default=6, help="Number of matrices (powers) (default: 6)")
    ...
    parser.add_argument("--s", type=int, default=1, help="Swap size parameter s (default: 1)")
    parser.add_argument("--max_iter_cap", type=int, default=600, 
                       help="Safety cap on iterations (default: 600)")
    parser.add_argument("--latex", action='store_true', 
                       help="Enable LaTeX text rendering (requires TeX installation)")
    
    args = parser.parse_args()
    run_experiment(d=args.d, n=args.n, n_iter=args.iters, s=args.s, 
                  epsilon=args.epsilon, max_iter_cap=args.max_iter_cap, latex=args.latex)
