"""
Run two-layer algorithm on Curie-Weiss model.
"""
import torch
import sys
import os
...
def run_cw_two_layer_experiment(
    d: int = 5,
    n_models: int = 5,
    ...
    """
    Run two-layer algorithm experiment on Curie-Weiss model.
    ...
    # Create Curie-Weiss base model and derive a family
    print("Creating Curie-Weiss Glauber model...")
    P_base, states, state_to_idx, H_vals, pi_base = curie_weiss_glauber(d, T=10.0, h=1.0, device=device)
    ...
    # Use readable plotting style (no external LaTeX dependency required)
    ...
    if save_results:
        ...
        # Save history (align with BL runner)
...
if __name__ == "__main__":
    # Run experiment
    S, w, f, history = run_cw_two_layer_experiment(
        d=5,
        n_models=5,
        l_cardinality=4,
        K_inner=30,
